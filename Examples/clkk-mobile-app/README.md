...
```bash
clkk-mobile-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
clkk-mobile-app$ sam local start-api
clkk-mobile-app$ curl http://localhost:3000/
```
...
```bash
clkk-mobile-app$ sam logs -n HelloWorldFunction --stack-name clkk-mobile-app --tail
```
...
```bash
clkk-mobile-app$ cd hello-world
...
```bash
sam delete --stack-name clkk-mobile-app
```
